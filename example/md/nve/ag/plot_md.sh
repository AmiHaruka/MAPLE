...
#
# 漂移率计算说明：
#   前 20% 的轨迹作为平衡段自动跳过，仅对生产窗口做线性回归。
#   NVE:     去除 NVT→NVE 切换时热浴 O-step（Leimkuhler & Matthews 2013 AMRX）
#            引起的热弛豫瞬变（ΔTE 可达数十 kcal/mol，持续 ~10–50 步）。
#   NVT/NPT: 去除早期热化斜坡，仅反映稳态行为。
#   一致于 logger.py 中的处理方式。
#
# 参考文献：
#   [1] Páll et al., J. Chem. Theory Comput. 2020 — 能量漂移率标准
#   [2] Case et al., AMBER 2023 Manual §3   — NVE 能量涨落标准；§3.1 平衡段丢弃
#   [3] Shirts & Chodera, JCP 129, 124105 (2008) — KE-PE 相关系数
#   [4] Allen & Tildesley, Computer Simulation of Liquids 2nd ed. 2017 §2.4, §6.1
#   [5] Frenkel & Smit, Understanding Molecular Simulation 2nd ed. 2002, Box 4.1
#   [6] Basconi & Shirts, JCTC 9, 2887 (2013) — NVT thermostat benchmarking
#   [7] Bussi et al., JCP 126, 014101 (2007) — V-rescale thermostat
#   [8] GROMACS Reference Manual 2024 §3.4.2 — NVE 平衡段丢弃建议
...
# ─── 统计量 ───────────────────────────────────────────────────────────────
Ha_to_kcal = 627.5095   # 1 Ha = 627.5 kcal/mol
fs_to_ns   = 1e-6
kB         = 8.617333e-5   # eV/K，仅作单位参考；计算用 Ha 内部一致

# ── 平衡段跳过（前 EQ_FRAC，NVE 和 NVT/NPT 均适用）──────────────────────
# NVE: 去除 NVT→NVE 切换后 BAOAB O-step 引起的热弛豫瞬变。
#      Leimkuhler & Matthews (2013) AMRX：BAOAB 不守恒 TE；切换瞬间 KE
#      是热浴最后一次随机踢后的值，与当前 PE 不构成微正则守恒对。
#      Frenkel & Smit (2002) Box 4.1：切换后先跑短 NVE 再分析漂移。
#      GROMACS Manual 2024 §3.4.2：NVT→NVE 后最初几 ps 应作平衡段丢弃。
# NVT/NPT: 去除早期热化斜坡（AMBER 2023 §3.1 建议）。
EQ_FRAC  = 0.20
n_all    = len(xdata)
eq_cut   = max(1, int(n_all * EQ_FRAC))
xprod    = xdata[eq_cut:]
TE_prod  = TE[eq_cut:]
eq_time_ps  = (xdata[eq_cut - 1] - xdata[0]) * 1e-3 if time is not None else None
prod_time_ps = (xprod[-1] - xprod[0]) * 1e-3       if time is not None else None

# 仅对生产窗口拟合漂移（与 logger.py 完全一致）
coef_prod    = np.polyfit(xprod, TE_prod, 1)
te_fit_prod  = np.polyval(coef_prod, xprod)

# 全轨迹仍拟合用于图 1 残差背景显示（不作判据）
te_fit_full  = np.polyval(np.polyfit(xdata, TE, 1), xdata)

te_fluct    = TE.std() / abs(TE.mean())
# 漂移率：每原子 kcal/mol/ns（Páll 2020），基于生产窗口
if n_atoms:
    drift_kcal_ns_atom = coef_prod[0] * Ha_to_kcal / fs_to_ns / n_atoms
    drift_label        = "kcal/mol/ns/atom"
else:
    drift_kcal_ns_atom = coef_prod[0] * Ha_to_kcal / fs_to_ns
    drift_label        = "kcal/mol/ns (N unknown)"
# 残差基于生产窗口（与判据一致）
residual    = (TE_prod - te_fit_prod) * Ha_to_kcal * 1000   # cal/mol
rms_resid   = np.sqrt(np.mean(residual**2))
# 短轨迹标志：生产窗口 < 10 ps
is_short_prod = (prod_time_ps is not None and prod_time_ps < 10.0)
...
if eq_time_ps is not None:
    short_warn = "  ⚠ SHORT" if is_short_prod else ""
    print(f" Drift window: skipped first {eq_time_ps:.2f} ps ({int(EQ_FRAC*100)}%),"
          f" production = {prod_time_ps:.2f} ps{short_warn}")
    print(f"   [F&S 2002 Box 4.1; GROMACS 2024 §3.4.2; AMBER 2023 §3.1]")
print()

if is_nve:
    print(" [NVE] Energy Conservation (primary criterion):")
    print(f"   σ(TE)/|⟨TE⟩|    = {te_fluct:.2e}   [AMBER: < 1e-4]  {'✓' if ok_fluct else '✗'}")
    # NVE: fine-grained drift thresholds (Páll 2020), on production window
    if np.isnan(drift_kcal_ns_atom):
        nve_drift_tag = 'N/A'
    elif is_short_prod:
        nve_drift_tag = 'SHORT (< 10 ps production window; indicative only)'
    elif abs(drift_kcal_ns_atom) < 0.01:
        nve_drift_tag = 'GOOD'
    elif abs(drift_kcal_ns_atom) < 0.1:
        nve_drift_tag = 'OK'
    elif abs(drift_kcal_ns_atom) < 1.0:
        nve_drift_tag = 'WARN'
    else:
        nve_drift_tag = 'FAIL'
    print(f"   Linear drift    = {drift_kcal_ns_atom:.4g} {drift_label}   [{nve_drift_tag}]  [Páll 2020: <0.01 GOOD, <0.1 OK, >1.0 FAIL]")
    if r_kepe is not None:
    ...
    print()
    # NVT/NPT: TE is not conserved by design; NVE thresholds (GOOD/OK) do NOT apply.
    # Only gross instability (> 1 kcal/mol/ns/atom) is meaningful here.
    # Refs: GROMACS Manual 2024 §3.4; Basconi & Shirts 2013 JCTC 9,2887
    if is_short_prod:
        nvt_drift_tag = 'SHORT (< 10 ps production window; indicative only)'
    elif abs(drift_kcal_ns_atom) > 1.0:
        nvt_drift_tag = 'WARN (gross instability)'
    else:
        nvt_drift_tag = 'PASS'
    print(f" [Reference] Energy Drift (secondary — NVT TE not conserved by design):")
    print(f"   σ(TE)/|⟨TE⟩|   = {te_fluct:.2e}   (NVT TE fluctuates by design; not an error metric)")
    print(f"   Linear drift   = {drift_kcal_ns_atom:.4g} {drift_label}   [{nvt_drift_tag}]")
    print(f"   (NVE GOOD/OK thresholds inapplicable; only drift > 1.0 signals gross instability)")
...
# 拟合线仅画在生产窗口（eq_cut 之后），平衡段用灰色底色标出
# ══════════════════════════════════════════════════════════════════════════
_prod_win_str = (f"prod {prod_time_ps:.1f} ps" if prod_time_ps is not None
                 else "prod window")
_short_flag   = "  ⚠SHORT" if is_short_prod else ""
...
if is_nve:
    ...
    title1 = (
        ...
        f'drift = {drift_kcal_ns_atom:.3g} {drift_label}'
    )

_drift_unit_short = "kcal/mol/ns/atom" if n_atoms else "kcal/mol/ns"
fig1, ax1 = new_fig(title1)
# 全轨迹数据点
ax1.plot(xdata, TE, color=BLUE, lw=0.9,
         marker='o', markersize=1.5, markevery=1, label='TE', zorder=2)
# 平衡段灰底
if eq_cut > 1:
    ax1.axvspan(xdata[0], xdata[eq_cut - 1],
                alpha=0.12, color=GRAY,
                label=f'Equilibration ({int(EQ_FRAC*100)}%)')
# 漂移拟合线仅画在生产窗口
ax1.plot(xprod, te_fit_prod, color=RED, lw=1.5, ls='--', zorder=3,
         label=f'Drift fit ({_prod_win_str}): {drift_kcal_ns_atom:.3g} {_drift_unit_short}{_short_flag}')
ax1.legend(fontsize=8, loc='upper right')
finish(ax1, 'E_total (Ha)')

# ══════════════════════════════════════════════════════════════════════════
# 图 2 — TE residual（生产窗口去线性漂移后的积分器噪声）
# ══════════════════════════════════════════════════════════════════════════
fig2, ax2 = new_fig(
    f'TE residual (drift removed, production window)  RMS = {rms_resid:.2g} J/mol'
    '  —  integrator noise'
)
ax2.plot(xprod, residual, color=GREEN, lw=0.9,
