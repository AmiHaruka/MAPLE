...
#            引起的热弛豫瞬变（ΔTE 可达数十 kJ/mol，持续 ~10–50 步）。
...
# 漂移率：每原子 kJ/mol/ns（Páll 2020），基于生产窗口
if n_atoms:
    drift_kJ_ns_atom = coef_prod[0] * Ha_to_kJ / fs_to_ns / n_atoms
    drift_label      = "kJ/mol/ns/atom"
else:
    drift_kJ_ns_atom = coef_prod[0] * Ha_to_kJ / fs_to_ns
    drift_label      = "kJ/mol/ns (N unknown)"
# 残差基于生产窗口（与判据一致）
residual    = (TE_prod - te_fit_prod) * Ha_to_kJ * 1000   # J/mol
...
if is_nve:
    ...
    if np.isnan(drift_kJ_ns_atom):
        nve_drift_tag = 'N/A'
    elif is_short_prod:
        nve_drift_tag = 'SHORT (< 10 ps production window; indicative only)'
    elif abs(drift_kJ_ns_atom) < 0.01:
        nve_drift_tag = 'GOOD'
    elif abs(drift_kJ_ns_atom) < 0.1:
        nve_drift_tag = 'OK'
    elif abs(drift_kJ_ns_atom) < 1.0:
        nve_drift_tag = 'WARN'
    else:
        nve_drift_tag = 'FAIL'
    print(f"   Linear drift    = {drift_kJ_ns_atom:.4g} {drift_label}   [{nve_drift_tag}]  [Páll 2020: <0.01 GOOD, <0.1 OK, >1.0 FAIL]")
    if r_kepe is not None:
    ...
    # Only gross instability (> 1 kJ/mol/ns/atom) is meaningful here.
    ...
    print(f"   Linear drift   = {drift_kJ_ns_atom:.4g} {drift_label}   [{nvt_drift_tag}]")
...
ax1.plot(xprod, te_fit_prod, color=RED, lw=1.5, ls='--', zorder=3,
         label=f'Drift fit ({_prod_win_str}): {drift_kJ_ns_atom:.3g} {_drift_unit_short}{_short_flag}')
