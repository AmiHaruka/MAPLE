#!/usr/bin/env python3
"""
Test helper for cluster:
1) parse a PDB file
2) locate target residue by chain + residue id (and optional model/icode)
3) build local cluster fragment (core + environment + caps + dangling-H)
4) export cluster as JSON and PDB
"""
...
try:
    from .cluster import ClusterFragment, build_cluster_fragment
...
def _jsonify(obj: Any) -> Any:
    if isinstance(obj, ResidueKey):
        return key_to_dict(obj)
    if isinstance(obj, Atom):
        return atom_to_dict(obj)
    if isinstance(obj, dict):
        # Keep dict keys JSON-friendly and stable.
...
def fragment_to_dict(fragment: ClusterFragment) -> dict[str, Any]:
    return {
        ...
        "context": _jsonify(fragment.context),
...
def _ordered_fragment_serials(fragment: ClusterFragment) -> list[int]:
    atom_by_serial = {atom.serial: atom for atom in fragment.atoms}
    preferred = ("ACE", "CORE", "TARGET", "ENV", "NME", "BOUNDARY_H")
    ordered_group_names = [name for name in preferred if name in fragment.residue_groups]
    ordered_group_names.extend(
        sorted(name for name in fragment.residue_groups.keys() if name not in ordered_group_names)
    )

    seen: set[int] = set()
    ordered: list[int] = []
    for group_name in ordered_group_names:
        for serial in sorted(fragment.residue_groups.get(group_name, [])):
            if serial in atom_by_serial and serial not in seen:
                ordered.append(serial)
                seen.add(serial)

    for serial in sorted(atom_by_serial.keys()):
...
def write_fragment_pdb(fragment: ClusterFragment, out_path: str) -> None:
    atom_by_serial = {atom.serial: atom for atom in fragment.atoms}
    lines: list[str] = []

    for serial in _ordered_fragment_serials(fragment):
        lines.append(format_atom_line(atom_by_serial[serial]))
...
def build_default_paths(
    ...
    cutoff_label = f"{cutoff:.2f}".replace(".", "p")
    tag = f"cluster_{chain_label}_{resseq}_m{model_label}_i{icode_label}_r{cutoff_label}_{env_mode}"
    return (
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build capped local cluster from input PDB and target residue."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.0,
        help="Environment residue cutoff distance in Angstrom (default: 4.0)",
    )
    parser.add_argument(
        "--env-mode",
        choices=("full", "rgroup"),
        default="full",
        help="Environment representation mode (default: full)",
    )
    parser.add_argument(
        "--json-out",
        dest="json_out",
    ...
    parser.add_argument(
        ...
        dest="pdb_out",
        default=None,
        help="Output cluster PDB file path (default: auto naming)",
    )
    parser.add_argument(
        "--indent",
        type=int,
    ...
    json_default, pdb_default = build_default_paths(
        args.input_pdb, args.chain, args.resseq, args.model, args.icode, args.cutoff, args.env_mode
    )
    ...
    result = build_cluster_fragment(structure, target_idx, cutoff=args.cutoff, env_mode=args.env_mode)
