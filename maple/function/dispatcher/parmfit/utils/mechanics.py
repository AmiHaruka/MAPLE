...
class MMTopologyCache:
    ...
    proper_by_torsion_bond: dict[tuple[int, int], list[Dihedral]] = field(default_factory=dict)
    improper_by_center: dict[int, list[Improper]] = field(default_factory=dict)
...
def _torsion_bond(dihedral: Dihedral) -> tuple[int, int]:
    return _pair(dihedral.atoms[1], dihedral.atoms[2])


def _improper_center(improper: Improper) -> int:
    return improper.atoms[2]
...
def build_mm_topology_cache(paramset: CorrectionParameterSet) -> MMTopologyCache:
    ...
    proper_by_torsion_bond: dict[tuple[int, int], list[Dihedral]] = {}
    for dihedral in sorted(paramset.dihedrals, key=lambda item: item.atoms):
        proper_by_torsion_bond.setdefault(_torsion_bond(dihedral), []).append(dihedral)

    improper_by_center: dict[int, list[Improper]] = {}
    for improper in sorted(paramset.impropers, key=lambda item: item.atoms):
        improper_by_center.setdefault(_improper_center(improper), []).append(improper)

    return MMTopologyCache(
        excluded_12=excluded_12,
        excluded_13=excluded_13,
        scaled_14=scaled_14,
        proper_by_torsion_bond=proper_by_torsion_bond,
        improper_by_center=improper_by_center,
    )
...
def evaluate_mm_energy(
    ...
    if len(atoms) != len(paramset.mol2.atoms):
        raise ValueError(
            f"Atoms length ({len(atoms)}) does not match parameter set size ({len(paramset.mol2.atoms)})."
        )

    positions = atoms.get_positions()
    cache = topology_cache if topology_cache is not None else build_mm_topology_cache(paramset)
    zero_center = None if zero_proper_torsion_bond is None else _pair(*zero_proper_torsion_bond)
    ...
    for dihedral in paramset.dihedrals:
        if zero_center is not None and _torsion_bond(dihedral) == zero_center:
            continue
    ...
    for improper in paramset.impropers:
        if zero_improper_center is not None and _improper_center(improper) == zero_improper_center:
            continue
